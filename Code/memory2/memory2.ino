...
//int data;
int trigpin1=22,echopin1=23,trigpin2=24,echopin2=25,trigpin3=26,echopin3=27;
...
unsigned long int data = 0; // stores the incoming hex value
...
//counter for counting the number of times program pass through a functions
int s1on_Counter = -1;
...
//global "current time" variables for different movement functions
unsigned long int current_Time5 = 0;// for Servo1 on
...
//total time spend by the pgm in executing the functions
unsigned long int total_s1on_Time[100];
...
void go_Forward(){
  ...
  switch (i) {
    ...
    case 6:
      total_s1on_Time[s1on_Counter + 1] = (current_Time0 - current_Time5);
      s1on_Counter++;
      break;
  }
  seq_Array[seq] = 1;
...
void go_Leftturn(){
  ...
  switch (i) {
    ...
    case 6:
      total_s1on_Time[s1on_Counter + 1] = (current_Time1 - current_Time5);
      s1on_Counter++;
      break;
  }
...
void go_Rightturn(){
  ...
  switch (i) {
    ...
    case 6:
      total_s1on_Time[s1on_Counter + 1] = (current_Time2 - current_Time5);
      s1on_Counter++;
      break;
  }
...
void go_Backward(){
  ...
  switch (i) {
    ...
    case 6:
      total_s1on_Time[s1on_Counter + 1] = (current_Time3 - current_Time5);
      s1on_Counter++;
      break;
  }
...
void write_To_Permt_Mem(){
  ...
  if(seq_Array[i-1]==1){
   ...
   else if(seq_Array[i-1]==6){
    static byte e=0;
    EEPROM.write(2*i-1,(total_s1on_Time[e])/1000);  
    e++;
   }           
  }
}
...
void go_servo1on_Seq(int p) {
  //go in Stop sequence
  servo1on();
  delay(total_s1on_Time[p]);
}
